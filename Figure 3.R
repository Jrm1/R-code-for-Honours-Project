...
#segments(sigma[1],mu[1],sigma[2],mu[2],lty="solid")

MVP3=(sigma[2]^2+1*sigma[1]*sigma[2])/(sigma[1]^2+sigma[2]^2+2*sigma[1]*sigma[2])
ER3=MVP3*mu[1]+(1-MVP3)*mu[2]

points(0,ER3,pch=19)
#segments(sigma[1],mu[1],0,ER3,lty="solid")
#segments(sigma[2],mu[2],0,ER3,lty="solid")

#No shoritng region
redtrans <- rgb(255, 150, 255, 70, maxColorValue=255) 
polygon(c(0,sigma[1],sigma[2]),c(ER3,mu[1],mu[2]),col=redtrans,border=NA)
polygon(c(sigma[2],1,1,sigma[1]),c(mu[2],mu[2],mu[1],mu[1]),col=redtrans,border=NA)

#Borrowing to hold more than 100% in Asset 2 and shorting asset 1 - region
bluetrans <- rgb(20, 80, 230, 50, maxColorValue=255) 
polygon(c(solve(a=coef(line(m~s))[2],b=-coef(line(m~s))[1]),1,1,sigma[2]),c(0,0,mu[2],mu[2]),col=bluetrans,border=NA)

##Borrowing to hold more than 100% in Asset 1 and shorting asset 2 - region
polygon(c(sigma[1],1,1,solve(a=coef(line(m~s))[2],b=-coef(line(m~s))[1]+0.2)),c(mu[1],mu[1],mu[1]+0.5,0.2),col=bluetrans,border=NA)
...
text(0.04,mu[1]-0.03, "unfeasible", srt = 0, xpd = TRUE, pos = 3,cex=1)
text(0.04,mu[1]-0.13, "unfeasible", srt = 0, xpd = TRUE, pos = 3,cex=1)
text(0.3,ER3, "no short-selling", srt = 0, xpd = TRUE, pos = 3,cex=1)
text(0.3,mu[1]-0.13, "short-sell asset 1", srt = 0, xpd = TRUE, pos = 3,cex=1)
text(0.37,mu[1]+0.01, "short-sell asset 2", srt = 0, xpd = TRUE, pos = 3,cex=1)
...
mtext("Figure 3: Portfolio positions",pos = 3,cex=1)

#legend(0.27,0.03,"no short-sales Bs",fill=redtrans)
#legend(0.25,0.047,"EF",lty="solid",col="blue",lwd="2")
